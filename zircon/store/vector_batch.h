...
#ifndef ZIRCON_STORE_VECTOR_BATCH_H_
#define ZIRCON_STORE_VECTOR_BATCH_H_
...
namespace zircon {
    ...
    public:
        ...
        ~VectorBatch() {
            if (_data) {
                Allocator::get_instance().deallocate(_data, _capacity * _vector_byte_size);
                _data = nullptr;
            }
        }
        ...
        [[nodiscard]] turbo::Status init(std::size_t vector_byte_size, std::size_t n) {
            ...
            try {
                _data = Allocator::get_instance().allocate(_capacity * _vector_byte_size);
        ...
        [[nodiscard]] turbo::Span<uint8_t> at(std::size_t i) const {
            ...
            TLOG_CHECK(turbo::simd::is_aligned(_data + i * _vector_byte_size), "not aligned {}",
                       turbo::ptr(_data + i * _vector_byte_size));
...
#endif  // ZIRCON_STORE_VECTOR_BATCH_H_
